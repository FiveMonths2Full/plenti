...
export default function DonatePage({ preferredSlug }: Props) {
  const { banks, selected, activeBankId, setActiveBankId, donorSession } = useStore()
  const [tab, setTab] = useState<Tab>('needs')
  const [toast, setToast] = useState({ visible: false, message: '' })

  // When a slug is provided (per-bank URL), auto-select that bank once banks load
  useEffect(() => {
    if (!preferredSlug || !banks.length) return
    const match = banks.find(b => b.slug === preferredSlug)
    if (match) setActiveBankId(match.id)
  }, [preferredSlug, banks, setActiveBankId])

  // Legacy ?bank=ID deep-link support
  useEffect(() => {
  ...
  const handleShare = useCallback(() => {
    const slug = activeBank?.slug
    const url = slug
      ? `${window.location.origin}/donate/${slug}`
      : `${window.location.origin}/donate?bank=${activeBankId}`
    navigator.clipboard?.writeText(url).catch(() => {})
    showToast('Share link copied to clipboard')
    trackEvent('share_tapped', { bank_id: activeBankId })
  }, [activeBank, activeBankId, showToast])
